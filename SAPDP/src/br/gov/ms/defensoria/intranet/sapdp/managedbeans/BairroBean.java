...
import br.gov.ms.defensoria.intranet.sapdp.lazymodels.BairroLazyModel;
...
public class BairroBean {
	...
	private LazyDataModel<Bairro> bairrosLazy;
	...
	public LazyDataModel<Bairro> getAllBairro(){
		...
		if(bairrosLazy == null){			
			bairrosLazy = new BairroLazyModel(mapa.get("codMunicipio"));
		}
		return bairrosLazy;
	}
	...
	public void insert() {
		...
		System.out.println(FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("nomeMunicipio"));
		this.bairro.setMunicipio(this.municipioDistritoBean.buscarMunicipioDistrito(FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("nomeMunicipio")));
	...
	public Bairro insert2(String nomeBairro) {
		FacesContext fc = FacesContext.getCurrentInstance();
		System.out.println(FacesContext.getCurrentInstance().getExternalContext().getRequestParameterMap().get("nomeMunicipio"));
		this.bairro.setMunicipio(this.municipioDistritoBean.buscarMunicipioDistrito(this.municipioDistritoBean.getMunicipioD().getNome()));
	...
	/**
	 * Lista os nomes dos bairros que contem parte do parametro
	 * informado
	 * @param query - parametro informado pelo usuário
	 * @return List<String>
	 */
	...
	/**
	 * Lista os nomes dos bairros e seus municípios que contem parte do parametro
	 * informado
	 * @param query - parametro informado pelo usuário
	 * @return List<String>
	 */
	...
	public void onItemSelect(SelectEvent event) {
        ...
        this.bairro = this.mRep.buscarBairroPorNome(event.getObject().toString());
	}
	
	/**
	 * Método para tratar o evento de seleção SelectEvent cujo event retorna string Bairro - Municipio
	 * @param event
	 */
	public void onItemSelectBairroMunicipio(SelectEvent event) {
		String aux[] = event.getObject().toString().split("-");
        FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(event.getObject().toString() + " SELECIONADO!",event.getObject().toString()));
        System.out.println("XXXXXXXXXXXXXXX"+aux[0].trim()+"__"+aux[1].trim());
        this.bairro = this.mRep.buscarBairroPorNomeEMunicipio(aux[0].trim(),aux[1].trim());       
	}
	
	public void selecionarNomeDoMunicipio(){
		this.municipioDistritoBean.getMunicipioD().setNome(this.bairro.getMunicipio() == null ? "" : this.bairro.getMunicipio().getNome()); 
	}
	...
	public LazyDataModel<Bairro> getBairrosLazy() {
		return bairrosLazy;
	}

	public void setBairrosLazy(LazyDataModel<Bairro> bairrosLazy) {
